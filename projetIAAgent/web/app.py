...
def clear_results():
    db = SessionLocal()
    try:
        # Delete all sessions and candidates (cascaded in models)
        db.query(models.ScreeningSession).delete()
        db.commit()
        
        # Clear temp_cvs folder
        temp_path = os.path.join(ROOT_DIR, "temp_cvs")
        ...
        flash("Tous les résultats ont été supprimés et les fichiers temporaires vidés.")
    finally:
        db.close()
    return redirect(url_for("dashboard"))
...
async def process_single_cv(job_id, session_id, path, is_anonymized, job_profile_dict, filename_as_name=None):
    db = SessionLocal()
    try:
        ...
        if filename_as_name and not is_anonymized:
            name = filename_as_name
        else:
...
async def batch_dispatcher(job_id, session_id, saved_files_data, is_anonymized, job_profile_dict):
    tasks = [
        process_single_cv(job_id, session_id, item['path'], is_anonymized, job_profile_dict, filename_as_name=item['display_name'])
        for item in saved_files_data
    ]
...
def screen_job():
    ...
    for cv_file in files:
        ...
        path = os.path.join("temp_cvs", f"{secrets.token_hex(4)}_{secure_name}")
        cv_file.save(path)
        saved_files_data.append({"path": path, "display_name": display_name})

    db.close()

    if saved_files_data:
        asyncio.run(batch_dispatcher(job_id, session.id, saved_files_data, is_anonymized, job_profile_dict))

    flash(f"{len(saved_files_data)} CVs analyses avec succes.")
    return redirect(url_for("dashboard"))
